...
def networkInsert(ts, sh, sp, dh, dp, pk, by):
    ...
    var = requests.get(url + "?ts=" + str(ts) + "src_host=" + str(sh) + "&src_port=" + str(sp) + "&dst_host=" + str(dh) + "&dst_port=" + str(dp) + "&pkts=" + str(pk) + "&bytes=" + str(by))
...
if __name__ == "__main__":
    ...
    while True:
        networkInsert(time.time(),"dummy1",1234,"dummy2",4321,15,3287)
        time.sleep(5)